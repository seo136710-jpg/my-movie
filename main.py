...
def main():
    ...
    # [데이터 가공 2: 누적관객 100만 이상 영화 트로피 🏆 표시]
    def add_trophy(row):
        name = row["movieNm"]
    ...
    # Streamlit 막대그래프 렌더링 방식에 맞춰 1위가 맨 위/왼쪽에 오도록 iloc[::-1] 처리
    top5_df = (
        df.head(5)[["movieNm", "audiCnt"]].iloc[::-1].set_index("movieNm")
    )
